...
vec3 mandelbrot( vec2 position )
{
	...
	vec2 m_offset = vlerp(position, julia_c, julia);
    ...
    {
        v = vlerp(cPow(v, exponent) + m_offset, cPow(vec2(abs(v.x), abs(v.y)), exponent) + m_offset, burning_ship);
        r2 = v.x * v.x + v.y * v.y;
		vec2 st = v;
		st *= 3.0;
		...
		if( f < min_distance ) {
			min_distance = f;
			trapped_at = iter;
			trap_angle = (atan(v.y, v.x) + PI)/(2 * PI);
		}
	...
	float smooth_trapped_at = float(trapped_at) - log(log(sqrt(r2)) / log(escape_radius)) / log(cAbs(exponent));
	smooth_trapped_at = log(smooth_trapped_at) / log(float(max_iterations));
	...
	float hue = fract(mix(rat, min_distance, lograt) * hue_stretch + hue_shift);
