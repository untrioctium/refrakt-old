...
public:
	...
	void run(refrakt::widget::param_t& input, refrakt::widget::param_t& output) const {
		...
		) * glm::translate(glm::scale(glm::mat4(1.0f), glm::vec3(scale)) * glm::eulerAngleYXZ(glm::radians(rot.x), glm::radians(rot.y), glm::radians(rot.z)), std::get<refrakt::vec3>(input["trans"]));
